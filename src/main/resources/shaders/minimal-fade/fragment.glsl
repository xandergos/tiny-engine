...
void main()
{
    ...
    float fade = clamp(1f - pow(dot(FragPos.xz, FragPos.xz)/(480*480), 4), 0, 1);
    FragColor = vec4(fade * (directional + pointSum + spotlight), texture(material.diffuse, TexCoord).w);
}
...
vec3 spotlightStrength()
{
    ...
    if(theta > spotlight.outerAngleCosine)
    {
        ...
        float attenuation = 1f-length(fragSourceOffset)/1000f;
